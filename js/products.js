...
TP.products = [
    ...
    {
        slug: "ropa-de-trabajo-alta-visibilidad/",
        ...
        price: "Desde $8", priceNote: "camiseta con reflectivo · resto según prenda",
...
TP.cardHTML = function (p, i) {
    var base = TP.base || "";
    var media = p.img
        ? '<img src="' + base + p.img + '" alt="' + p.name + ' — Textiles Pelileo" loading="lazy" width="600" height="450">'
    ...
    '<article class="card reveal ' + delay + '">' +
        '<a class="card__media" href="' + base + p.slug + '" aria-label="' + p.name + '">' +
            '<span class="badge ' + (p.badgeClass || "") + '">' + p.badge + '</span>' + media +
        '</a>' +
        '<div class="card__body">' +
            '<h3><a href="' + base + p.slug + '">' + p.name + '</a></h3>' +
            ...
            '<div class="card__foot">' +
                ...
                '<a class="btn btn--outline btn--block mt-1" href="' + base + p.slug + '">Ver detalles</a>' +
            '</div>' +
        '</div>' +
    '</article>';
};
